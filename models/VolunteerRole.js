...
volunteerRoleSchema.virtual('isOpen').get(function() {
  return this.status === 'open' && this.positionsFilled < this.numberOfPositions;
});
...
volunteerRoleSchema.pre('save', async function(next) {
  ...
  if (this.positionsFilled >= this.numberOfPositions && this.status === 'open') {
    this.status = 'filled';
  }
...
volunteerRoleSchema.methods.fillPosition = function(count = 1) {
  this.positionsFilled = Math.min(this.positionsFilled + count, this.numberOfPositions);
  return this.save();
};

volunteerRoleSchema.methods.unfillPosition = function(count = 1) {
  this.positionsFilled = Math.max(0, this.positionsFilled - count);
  if (this.status === 'filled' && this.positionsFilled < this.numberOfPositions) {
    this.status = 'open';
  }
  return this.save();
};
