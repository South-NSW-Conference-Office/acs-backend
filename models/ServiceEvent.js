...
const serviceEventSchema = new mongoose.Schema({
  ...
  eventType: {
    type: String,
    enum: ['workshop', 'training', 'fundraiser', 'community_meal', 'distribution', 'health_screening', 'other'],
    default: 'other'
  },
  start: {
    type: Date,
...
serviceEventSchema.virtual('isRegistrationOpen').get(function() {
  ...
  if (this.registration.deadline && this.registration.deadline < new Date()) return false;
  if (this.capacity.maximum && this.capacity.registered >= this.capacity.maximum) return false;
  return true;
});
...
serviceEventSchema.pre('save', async function(next) {
  if (this.isNew && !this.organization) {
    const service = await mongoose.model('Service').findById(this.service).select('organization');
    if (service) {
...
serviceEventSchema.statics.findByDateRange = function(startDate, endDate, filters = {}) {
  return this.find({
    ...filters,
    start: { $gte: startDate },
    end: { $lte: endDate },
