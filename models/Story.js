...
const storySchema = new mongoose.Schema({
  ...
  categories: [{
    type: String,
    enum: ['success_story', 'news', 'update', 'testimonial', 'case_study', 'annual_report'],
...
storySchema.index({ title: 'text', summary: 'text', body: 'text', tags: 'text' });
...
storySchema.virtual('isCurrentlyFeatured').get(function() {
  return this.isFeatured && (!this.featuredUntil || this.featuredUntil > new Date());
});
...
storySchema.pre('save', async function(next) {
  if (!this.slug) {
    ...
    let counter = 1;
    while (await mongoose.model('Story').findOne({ slug: this.slug, _id: { $ne: this._id } })) {
