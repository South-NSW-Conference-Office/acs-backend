  ...
  try {
    ...
    if (user.primaryOrganization && user.organizations.length > 0) {
      const primaryOrgAssignment = user.organizations.find(
        org => org.organization && org.organization._id && org.organization._id.toString() === user.primaryOrganization._id.toString()
      );
    ...
    const userData = {
      ...
      avatar: user.avatar,
      organizations: user.organizations.filter(org => org.organization && org.role).map(org => ({
        organization: {
...
router.post('/forgot-password', [
  ...
  try {
    ...
    try {
      console.log(`[FORGOT PASSWORD] Attempting to send email to: ${email} with token: ${resetToken}`);
