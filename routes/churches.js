...
/** Convert nested object to MongoDB dot-notation for safe $set updates */
...
/** Whitelist of fields clients may set via PUT */
...
/** Pick only allowed top-level keys from the request body */
...
function handleChurchError(error, res) {
  if (error.code === 11000) {
    ...
    return res.status(409).json({
      success: false,
      message: `A Church with that ${field} already exists`,
    });
  }
