...
router.get('/', [
  query('includeSystem').optional().isBoolean().withMessage('includeSystem must be a boolean'),
  query('isSystemRole').optional().isBoolean().withMessage('isSystemRole must be a boolean'),
  query('level').optional().isIn(['union', 'conference', 'church']).withMessage('level must be union, conference, or church')
], async (req, res) => {
  try {
    ...
    const includeSystem = req.query.includeSystem === 'true' || req.query.isSystemRole === 'true';
    ...
    const roles = await Role.find(query).sort({ isSystem: -1, level: 1, displayName: 1 });
...
router.post('/', [
  ...
  body('level').isIn(['union', 'conference', 'church']).withMessage('Level must be union, conference, or church'),
  body('permissions').optional().isArray().withMessage('Permissions must be an array'),
  body('description').optional().isString().trim(),
  body('isSystem').optional().isBoolean().withMessage('isSystem must be a boolean')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    const { name, displayName, level, permissions, description, isSystem } = req.body;
...
router.put('/:id', [
  ...
  body('level').optional().isIn(['union', 'conference', 'church']).withMessage('Level must be union, conference, or church'),
  body('permissions').optional().isArray().withMessage('Permissions must be an array'),
  body('description').optional().isString().trim(),
  ...
  try {
    ...
    if (role.isSystem && Object.keys(updates).some(key => key !== 'isActive')) {
    ...
    if (updates.name && updates.name.toLowerCase() !== role.name) {
      const existingRole = await Role.findOne({ name: updates.name.toLowerCase() });
      if (existingRole) {
        return res.status(400).json({
          success: false,
          message: 'Role with this name already exists',
